...
import { Award, TrendingUp, User, Trophy, Search, ChevronRight, Star } from "lucide-react";
        ...
        <div className="relative p-10 md:p-16 flex flex-col md:flex-row justify-between items-end gap-8">
          <div>
            ...
            <h1 className="text-5xl md:text-6xl font-black text-white tracking-tighter leading-[0.85] mb-4">
              PLAYER <br/><span className="text-orange-500">DATABASE</span>
            </h1>
          ...
          {filteredPlayers.map((player) => {
            ...
            return (
              <motion.div
                ...
                layout
                className="group relative bg-zinc-900 rounded-[2.5rem] overflow-hidden border border-zinc-800 transition-all duration-500 hover:border-orange-500/50 hover:shadow-2xl hover:shadow-orange-500/10 active:scale-[0.98]"
              >
                {/* Image Section */}
                <div className="relative h-72 overflow-hidden">
                  ...
                  <div className="absolute bottom-6 left-6 right-6">
                    <h3 className="text-3xl font-black text-white tracking-tighter leading-none mb-2 uppercase">{player.name}</h3>
                    <div className="flex items-center gap-3">
                      <span className="bg-white text-zinc-950 px-2 py-0.5 rounded text-[10px] font-black uppercase">
                        {player.position}
                      </span>
                      <span className="text-zinc-300 text-xs font-bold uppercase tracking-widest">Grade {player.grade}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-8">
                  <Link
                    to={`/team/${team?.id}`}
                    className="flex items-center gap-3 mb-8 group/team"
                  >
                    <img
                      src={team?.image}
                      alt=""
                      className="w-10 h-10 rounded-xl object-cover border-2 border-zinc-800 group-hover/team:border-orange-500 transition-all shadow-md"
                    />
                    <div>
                      <div className="font-black text-sm text-white group-hover/team:text-orange-400 transition-colors uppercase tracking-tight">{team?.name}</div>
            ...
            <div key={leaderGroup.pos}>
              ...
              <div className="space-y-4">
                {players
                  ...
                  .slice(0, 3)
                  .map((player, idx) => (
                    <div key={player.id} className="flex items-center gap-4 bg-zinc-950/50 rounded-2xl p-4 border border-zinc-800/50 hover:border-zinc-700 transition-colors">
